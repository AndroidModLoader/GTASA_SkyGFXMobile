...
void CreateEffectsShaders()
{
    ...
    char sVignettePxl[] = "precision highp float;\n"
                          ...
                          "}\n"
                          "void main() {\n"
                          "  float vignetteValue = 1.0 - vignette(Out_Tex0, 0.35, 0.5);\n"
                          "  vec4 color = texture2D(Diffuse, Out_Tex0) * vignetteValue;\n"
                          "  color.a *= GFX1v.x;"
                          "  gl_FragColor = color;\n"
                          "}";
    ...
    char sRBlurPxl[] = "precision mediump float;\n"
                       ...
                       "uniform mediump vec4 GFX1v;\n"
                       "void main() {\n"
                       "  vec3 c = texture2D(Diffuse, Out_Tex0 - 0.5 * GFX1v.xy).rgb;\n"
                       "  c += texture2D(Diffuse, Out_Tex0 - 0.25 * GFX1v.xy).rgb;\n"
                       "  c += texture2D(Diffuse, Out_Tex0).rgb;\n"
                       "  c += texture2D(Diffuse, Out_Tex0 + 0.25 * GFX1v.xy).rgb;\n"
                       "  c += texture2D(Diffuse, Out_Tex0 + 0.5 * GFX1v.xy).rgb;\n"
